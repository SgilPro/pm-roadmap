...
職缺現況見 `index.html` 的 `JOBS_DATA`——**那邊是 SSOT，這裡是快照**。

2026-08-12 對過一次：25sprout 已退場（下架滿 7 天後由 `check-jobs.mjs` 從 `JOBS_DATA` 移除，累計退場 10 / 23）。GoFreight 仍開放但已從 `top` 降為 `track`，題組留著——英文練習的價值不隨 tier 變。剩下的 4 個 `top` 全部 `health: "open"`。

> 某家退場了就把它的題組刪掉，不要留著練一個已經不存在的公司。**這份檔案的淨方向也是減少。**
